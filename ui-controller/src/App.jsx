...
export default function App() {
  ...
  const manipulatorChip = manip
    ? [
        manip.arm?.preset ?? (manip.arm ? "joints" : null),
        manip.gripper?.action,
      ]
        .filter(Boolean)
        .join(" \u00b7 ")
    : "arm idle";
  ...
  return (
    <div className="mx-auto flex min-h-dvh max-w-[1680px] flex-col gap-3 p-3 xl:h-dvh xl:overflow-hidden">
      <TopBar
        ...
        onReset={handleReset}
        simulationOpen={showSimulation}
        simulationActive={ctl.simulation.enabled}
        onToggleSettings={() => {
      ...
      {showSimulation && (
        <SimulationModal
          simulation={ctl.simulation}
          onStart={ctl.startSimulation}
          onStop={ctl.stopSimulation}
          onClose={() => setShowSimulation(false)}
          disabled={ctl.gatewayReady !== true || !ctl.world.robot}
        />
      )}
      ...
      {ctl.gatewayReady === false && view === "console" && (
        <div
          role="status"
          className="a-rise flex shrink-0 items-start gap-2.5 rounded-xl
          ...
          <p className="text-[11.5px] leading-relaxed text-dim">
            ...
            <span className="font-mono">/api/teleop/config</span> is not
            answering on <span className="font-mono">{cfg.api}</span>.
            Scenarios, telemetry and investigation still work; zone geometry
            below is the contract default.
          </p>
        </div>
      )}
      ...
      {revoked && (
        <div
          ...
          <ShieldAlert
            size={14}
            className="mt-px shrink-0 text-bad"
            aria-hidden="true"
          />
          <p className="text-[11.5px] leading-relaxed text-dim">
            <b className="text-bad">Credential revoked.</b> The hacker burned
            the shared fleet credential, so the valid operator is locked out too
            — the blast radius of a shared credential. Press{" "}
      ...
      {view === "logs" ? (
        ...
        <>
            ...
            <div className="card flex min-h-0 flex-col p-3.5">
              <div className="mb-2 flex shrink-0 flex-wrap items-center justify-between gap-2">
                <h2 className="flex items-center gap-1.5 text-[13.5px]">
                  <Radar size={13} className="text-info" aria-hidden="true" />
                  Warehouse floor
                </h2>
                <div className="flex items-center gap-1.5">
                  <span
                    className={`chip ${ctl.simulation.enabled ? "border-info/45 text-info" : ""}`}>
                    {ctl.simulation.enabled && (
                      <Bot size={9} aria-hidden="true" />
                    )}
                    {ctl.simulation.enabled
                      ? ctl.simulation.phase === "override"
                ...
                <div className="mb-1 flex shrink-0 items-center justify-between gap-2">
                  ...
                  <span className="chip">
                    ...
                    <span className="text-faint">
                      · d-pad arm · L1/L2 · R1/R2 grip
                    </span>
                  </span>
                </div>
                <div className="min-h-0 flex-1">
                  <DualSense
                    stickRef={ctl.stickRef}
          ...
          <section
            className="a-stagger grid shrink-0 gap-3 xl:h-[228px]
