  ...
  return (
    <button
      ...
      className={`inline-flex cursor-pointer items-center gap-1.5 rounded-lg px-2.5 py-1.5
                  ...
                  ${
                    active
                      ? "bg-elevated text-txt shadow-[0_1px_0_rgba(255,255,255,.06)_inset]"
    ...
    <header className="flex shrink-0 flex-wrap items-center gap-3">
      <div className="flex items-center gap-2.5">
        <div
          ...
          style={{
            background:
              "conic-gradient(from 140deg,#1d4ed8,#059669,#a78bfa,#1d4ed8)",
      ...
      <nav
        className="flex items-center gap-1 rounded-xl border border-line bg-sunken/70 p-1"
        aria-label="Views">
        <Tab
          active={view === "console"}
      ...
      <div className="flex flex-1 flex-wrap items-center gap-1.5">
        <span
          ...
          <span className="relative flex size-1.5">
            {online && (
              <span className="a-ping absolute inset-0 rounded-full bg-ok" />
            )}
            <span
              className={`relative size-1.5 rounded-full ${online ? "bg-ok" : "bg-bad"}`}
            />
          </span>
        ...
        <span
          title="Backend-mediated teleoperation gateway"
          className={`chip ${
            gatewayReady === true
        ...
        {ai && (
          <span
            title={`artifact ${ai.artifact_verified ? "verified" : "UNVERIFIED"} · ${ai.n_training_samples} samples`}
      ...
      <div className="flex items-center gap-1.5">
        <button
          ...
          className={`btn btn-sm ${simulationActive ? "border-info/45 text-info" : ""}`}>
          <Bot size={12} aria-hidden="true" />
          Simulation
          {simulationActive && (
            <span
              className="size-1.5 rounded-full bg-info a-pulse"
              aria-label="active"
            />
          )}
        </button>
